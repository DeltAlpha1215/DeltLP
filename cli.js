...
// ─── Load .env from ~/.deltlp/ if present ──────────────────────
const deltlpDir = path.join(os.homedir(), ".deltlp");
const deltlpEnv = path.join(deltlpDir, ".env");
if (fs.existsSync(deltlpEnv)) {
  loadEnv({
    envPath: deltlpEnv,
    keyPath: path.join(deltlpDir, ".envrypt"),
    override: false,
  });
}
...
// ─── SKILL.md generation ──────────────────────────────────────────
const SKILL_MD = `# deltlp — Solana DLMM LP Agent CLI
...
### deltlp screen [--dry-run] [--silent]
Runs one AI screening cycle to find and deploy new positions.
\`\`\`
Output: { done: true, report: "..." }
\`\`\`

### deltlp manage [--dry-run] [--silent]
Runs one AI management cycle over open positions.
\`\`\`
Output: { done: true, report: "..." }
\`\`\`

### deltlp deploy --pool <addr> --amount <sol> [--bins-below 69] [--bins-above 0] [--strategy bid_ask|spot] [--dry-run]
...
### deltlp search-pools --query <name_or_symbol> [--limit 10]
...
### deltlp blacklist add --mint <addr> --reason <text>
...
fs.mkdirSync(deltlpDir, { recursive: true });
fs.writeFileSync(path.join(deltlpDir, "SKILL.md"), SKILL_MD);
...
switch (subcommand) {
  ...
  case "pnl": {
    ...
    if (!positionAddress) die("Usage: deltlp pnl <position_address>");
  ...
  case "token-info": {
    ...
    if (!query) die("Usage: deltlp token-info --query <mint_or_symbol>");
  ...
  case "token-holders": {
    ...
    if (!mint) die("Usage: deltlp token-holders --mint <addr>");
  ...
  case "token-narrative": {
    ...
    if (!mint) die("Usage: deltlp token-narrative --mint <addr>");
  ...
  // ── pool-detail ───────────────────────────────────────────────
  case "pool-detail": {
    if (!flags.pool) die("Usage: deltlp pool-detail --pool <addr> [--timeframe 5m]");
  ...
  case "search-pools": {
    ...
    if (!query) die("Usage: deltlp search-pools --query <name_or_symbol>");
  ...
  // ── active-bin ────────────────────────────────────────────────
  case "active-bin": {
    if (!flags.pool) die("Usage: deltlp active-bin --pool <addr>");
    const { getActiveBin } = await import("./tools/dlmm.js");
  ...
  case "wallet-positions": {
    ...
    if (!wallet) die("Usage: deltlp wallet-positions --wallet <addr>");
  ...
  // ── deploy ───────────────────────────────────────────────────────
  case "deploy": {
    if (!flags.pool) die("Usage: deltlp deploy --pool <addr> --amount <sol>");
  ...
  // ── claim ────────────────────────────────────────────────────────
  case "claim": {
    if (!flags.position) die("Usage: deltlp claim --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── close ────────────────────────────────────────────────────────
  case "close": {
    if (!flags.position) die("Usage: deltlp close --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── swap ─────────────────────────────────────────────────────────
  case "swap": {
    if (!flags.from || !flags.to || !flags.amount) die("Usage: deltlp swap --from <mint> --to <mint> --amount <n>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  case "config": {
    if (sub2 === "get" || !sub2) {
      ...
      if (!key || rawVal === undefined) die("Usage: deltlp config set <key> <value>");
      let value = rawVal;
  ...
  // ── study ────────────────────────────────────────────────────────
  case "study": {
    if (!flags.pool) die("Usage: deltlp study --pool <addr> [--limit 4]");
  ...
  case "start": {
    ...
    process.stderr.write("[deltlp] Starting autonomous agent...\n");
    startCronJobs();
    break;
  }
  ...
  case "lessons": {
    if (sub2 === "add") {
      ...
      if (!text) die("Usage: deltlp lessons add <text>");
  ...
  // ── pool-memory ──────────────────────────────────────────────────
  case "pool-memory": {
    if (!flags.pool) die("Usage: deltlp pool-memory --pool <addr>");
  ...
  case "blacklist": {
    if (sub2 === "add") {
      if (!flags.mint) die("Usage: deltlp blacklist add --mint <addr> --reason <text>");
  ...
  case "withdraw-liquidity": {
    if (!flags.position) die("Usage: deltlp withdraw-liquidity --position <addr> --pool <addr> [--bps 10000]");
  ...
  // ── add-liquidity ──────────────────────────────────────────────
  case "add-liquidity": {
    if (!flags.position) die("Usage: deltlp add-liquidity --position <addr> --pool <addr> [--amount-x <n>] [--amount-y <n>]");
    if (!flags.pool) die("--pool is required");
  ...
  default:
    die(`Unknown command: ${subcommand}. Run 'deltlp help' for usage.`);
}
